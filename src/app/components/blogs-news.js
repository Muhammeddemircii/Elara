                    ...
                    {data.map((item, index) => (
                        <div className="blog relative rounded-md shadow shadow-slate-200 dark:shadow-gray-800 overflow-hidden" key={index}>
                            ...
                            <div className="content p-6">
                                ...
                                <div className="mt-5">
                                    <Link href={`blog-detail/${item.id}`} className="hover:text-violet-600 dark:hover:text-violet-600 after:bg-violet-600 dark:text-white transition duration-500 inline-flex items-center gap-1 font-montserrat">{item.more}<Unicons.UilArrowRight width={16}/></Link>
                                </div>
                            </div>
                        </div>
                    ))}
